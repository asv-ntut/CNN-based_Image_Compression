...
5. Socket IPC: 作為背景常駐服務，免去模型反覆載入開銷，等待 C Dispatcher 派發任務。

Usage (背景啟動):
    python compress.py --enc encoder.onnx --hyper hyper.onnx --batch 16 --workers 4 &
"""
import argparse
import os
import sys
...
# ==============================================================================
# 4.5 IPC Server & Network Communication
# ==============================================================================
def send_data_to_comm(output_dir, comm_ip, comm_port):
    """將產生出來的 Patch .bin 檔傳送給通訊團隊"""
    print(f"\n📡 準備將壓縮封包 {output_dir} 傳送至通訊團隊 {comm_ip}:{comm_port}...")
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            print(f"✅ 網路傳輸成功！共傳送了 {file_count} 個 Patch 封包。")
    except Exception as e:
        print(f"⚠️ 網路傳輸發生錯誤: {e}")

def start_ipc_server(sess, mods, executor, args):
    """啟動本機 Socket Server，等待 C Dispatcher 派發任務"""
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) 
    server_socket.bind(('127.0.0.1', 6000))
    server_socket.listen(1)
    
    print(f"\n🟢 AI Worker 已就緒，等待本機 C Dispatcher 呼叫 (Port 6000)...")
    ...
    while True:
        ...
        with conn:
            ...
            try:
                ...
                mode = task.get('mode')
                image_path = task.get('file_path')
                
                print(f"\n📥 [IPC 收到任務] Mode={mode}, Target={image_path}")
                
                if not os.path.exists(image_path):
                    print(f"❌ 找不到指定的影像檔案: {image_path}")
                    conn.sendall(b"ERROR: FILE_NOT_FOUND")
                    continue

                # 處理 Mode 3 (保留擴充空間)
                if mode == 3:
                    print("⚙️ 執行 Mode 3: 進入 Residual 前置處理...")
                    # image_path = run_residual_preprocess(image_path)
                
                # 執行核心壓縮
                compress_image(sess, mods, executor, image_path, args.output_dir, 1, args.batch, 256)
                
                # 網路傳輸
                fname = os.path.basename(image_path)
                sdir = os.path.join(args.output_dir, os.path.splitext(fname)[0])
                send_data_to_comm(sdir, args.comm_ip, args.comm_port)
...
def main():
    parser = argparse.ArgumentParser(description="Satellite TIC Background Worker")
    parser.add_argument("-o", "--output_dir", default="output_bin")
    ...
    parser.add_argument("--comm_ip", default="192.168.70.1", help="通訊團隊 IP")
    parser.add_argument("--comm_port", type=int, default=5000, help="通訊團隊 Port")
    args = parser.parse_args()
    
    print("🚀 Initializing AI Worker Environment...")
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        start_ipc_server(sess, mods, executor, args)
