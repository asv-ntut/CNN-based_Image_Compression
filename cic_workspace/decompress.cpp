...
int main(int argc, char** argv) {
    try {
        const std::string DEFAULT_TARGET_NAME  = "test.tif"; 
        const std::string DEFAULT_BIN_DIR      = "./compressed_bins";
        const std::string MODEL_DECODER        = "./model/onnx/cic_decoder.onnx";
        const std::string MODEL_HYPER_DECODER  = "./model/onnx/cic_hyper_decoder.onnx";

        std::string target_name  = (argc > 1) ? argv[1] : DEFAULT_TARGET_NAME;
        ...
        std::string file_stem = fs::path(target_name).stem().string();
        ...
        std::cout << "[INFO] Hardcoding image dimensions (4096x3072)..." << std::endl;
